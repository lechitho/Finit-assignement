...
namespace Finit.Application.Services
{
    public class TaskService : ITaskService
    {
        ...
        public async Task<TaskViewModel> Create(TaskViewModel taskViewModel)
        {
            using (var scope = _tracer.BuildSpan("Create_TaskService").StartActive(true))
            {
                ...
                var taskResult = await _mediator.SendAsync<TaskInfo>(newTaskCommand);

                return _taskViewModelMapper.ConstructFromEntity(taskResult);
            }
        }
        public async Task<TaskViewModel> Update(TaskViewModel taskViewModel)
        {
            using (var scope = _tracer.BuildSpan("Update_TaskService").StartActive(true))
            {
                var updateTaskCommand = _taskViewModelMapper.ConvertToUpdateTaskCommand(taskViewModel);

                var taskResult = await _mediator.SendAsync<TaskInfo>(updateTaskCommand);
